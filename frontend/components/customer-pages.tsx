...
export function ProductsPage({ productId }: { productId?: number }) {
  ...
  if (productId && product) {
    return <main className="public-page"><PublicHeader /><section className="product-detail"><div className="detail-visual"><span>{product.category_name ?? "精选好物"}</span><strong className="category-symbol">{categoryIcon(product.category_name)}</strong></div><div className="detail-copy"><Link href="/products">← 返回全部商品</Link><p className="eyebrow">{product.store_name ?? "精选店铺"}</p><h1>{product.name}</h1><p>{product.description || "用心挑选的品质好物。"}</p><div className="product-tags detail-tags">{product.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="detail-price">{money(product.current_price)}</div><div className="detail-market-meta"><span>{product.store_name ?? "CommerceHub 精选"}</span><span>{product.sales_count > 0 ? `已售 ${product.sales_count} 件` : "新品上架"}</span></div><div className="stock-line"><StatusBadge value={product.inventory_quantity > 0 ? "IN_STOCK" : "OUT_OF_STOCK"} />{product.inventory_quantity > 0 ? `还剩 ${product.inventory_quantity} 件` : "补货中"}</div><button className="primary" onClick={() => addToCart(product.id)} disabled={!product.inventory_quantity}>加入购物车</button><Notice message={message} tone={message.includes("已加入") ? "success" : "error"} /></div></section></main>;
  }
  ...
  return (
    <main className="public-page"><PublicHeader /><section className="catalog-page" id="catalog">
      ...
      {loading ? <LoadingState /> : products.length === 0 ? <EmptyState>暂时没有找到合适的商品，换个条件试试吧。</EmptyState> : <div className="product-grid">{products.map((item) => <article className="product-card" key={item.id}><Link className="product-visual" href={`/products/${item.id}`}><span>{item.category_name ?? "精选好物"}</span><strong className="category-symbol">{categoryIcon(item.category_name)}</strong></Link><div className="product-body"><div className="product-tags">{item.tags.slice(0, 3).map((tag) => <span key={tag}>{tag}</span>)}</div><Link href={`/products/${item.id}`}><h2>{item.name}</h2></Link><p>{item.description || "用心挑选的品质好物。"}</p><div className="product-meta"><strong>{money(item.current_price)}</strong><span>{item.inventory_quantity > 5 ? "现货" : item.inventory_quantity > 0 ? `仅剩 ${item.inventory_quantity} 件` : "暂时缺货"}</span></div><div className="product-store"><span>{item.store_name ?? "CommerceHub 精选"}</span><span>{item.sales_count > 0 ? `已售 ${item.sales_count} 件` : "新品"}</span></div><button onClick={() => addToCart(item.id)} disabled={!item.inventory_quantity}>{item.inventory_quantity ? "加入购物车" : "暂时缺货"}</button></div></article>)}</div>}
    </section></main>
  );
}
...
function OrderDetail({ order }: { order: Order }) {
  return <div className="split-grid"><section className="panel"><div className="panel-heading"><div><p className="eyebrow">订单号 {order.order_no}</p><h2>{formatDate(order.created_at)}</h2></div><StatusBadge value={order.status} /></div><div className="table-scroll"><table><thead><tr><th>商品</th><th>单价</th><th>数量</th><th>小计</th></tr></thead><tbody>{order.items.map((item) => <tr key={item.id}><td>{item.product_name_snapshot}</td><td>{money(item.unit_price)}</td><td>{item.quantity}</td><td>{money(item.subtotal)}</td></tr>)}</tbody></table></div><div className="summary-total"><span>订单合计</span><strong>{money(order.total_amount)}</strong></div></section><section className="panel"><h2>配送与支付</h2><p>{Object.values(order.address_snapshot).filter(Boolean).join(" · ")}</p><div className="order-items">{order.payments.length ? order.payments.map((payment) => <span key={payment.id}>{payment.method === "MOCK_CARD" ? "银行卡" : "电子钱包"} · {money(payment.amount)} · {payment.status === "SUCCESS" ? "支付成功" : payment.status}</span>) : <span>尚无支付记录</span>}</div></section></div>;
}
