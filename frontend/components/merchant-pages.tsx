...
const titles: Record<MerchantView, [string, string]> = {
  ...
  orders: ["订单处理", "按待处理、处理中、已发货的顺序完成履约。"],
  analytics: ["经营分析", "根据已完成交易汇总店铺销售表现。"],
};
...
export function MerchantPage({ view }: { view: MerchantView }) {
  ...
  async function createProduct(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api("/api/merchant/products", { method: "POST", body: JSON.stringify({ ...values, tags }) }); event.currentTarget.reset(); setMessage("商品已创建为草稿"); load(); } catch (error) { setMessage(errorMessage(error)); }
  }
...
export function MerchantProductForm({ productId }: { productId?: number }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const tags = String(values.tags ?? "").split(/[,，]/).map((tag) => tag.trim()).filter(Boolean);
    const body = { ...values, category_id: Number(values.category_id), tags };
    try {
