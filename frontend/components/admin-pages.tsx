...
export function AdminPage({ view }: { view: AdminView }) {
  ...
  function render() {
    if (view === "dashboard") {
      ...
      return <><div className="metric-grid">{metrics.map(([label, value]) => <article className="metric-card" key={String(label)}><span>{label}</span><strong>{value}</strong></article>)}</div><div className="dashboard-grid"><section className="panel revenue-panel"><p className="eyebrow">PLATFORM GMV</p><h2>{money(summary?.total_revenue ?? 0)}</h2><p>来自 {summary?.paid_order_count ?? 0} 笔有效成交订单</p><div className="summary-line"><span>平均订单金额</span><strong>{money(summary?.average_order_amount ?? 0)}</strong></div></section><section className="panel"><div className="panel-heading"><h2>平台构成</h2><span>实时数据库计数</span></div><div className="composition"><div><b>{summary?.store_count ?? 0}</b><span>店铺</span></div><div><b>{summary?.merchant_count ?? 0}</b><span>商家</span></div><div><b>{summary?.product_count ?? 0}</b><span>商品</span></div></div></section></div></>;
    }
